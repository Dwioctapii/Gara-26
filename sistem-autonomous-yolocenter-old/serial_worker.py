...
Protokol paket (6 byte):
  [0] 0xAA          — sync header
  [1] PWM low byte  — servo_pwm & 0xFF
  [2] PWM high byte — servo_pwm >> 8
  [3] mode byte     — 0=MANUAL 1=AUTO 2=HOLD 0xFF=DISCONNECT
  [4] detect byte   — enum status deteksi buoy YOLO:
                        0 = tidak ada buoy terdeteksi
                        1 = kedua buoy terdeteksi (buoyred + buoygreen)
                        2 = hanya buoyred terdeteksi
                        3 = hanya buoygreen terdeteksi
  [5] XOR checksum  — byte[1] ^ byte[2] ^ byte[3] ^ byte[4]

Strategi pengiriman:
  - Kirim SEGERA jika servo_pwm, mode, atau status deteksi buoy berubah (latensi max ~5ms)
...
# Mapping buoy.mode (dari vision_worker) → detect byte yang dikirim ke Teensy
#   "NONE"       → 0  : tidak ada buoy terdeteksi
#   "BOTH"       → 1  : buoyred + buoygreen terdeteksi
#   "RED_ONLY"   → 2  : hanya buoyred terdeteksi
#   "GREEN_ONLY" → 3  : hanya buoygreen terdeteksi
_DETECT_MAP = {
    "NONE":       0,
    "BOTH":       1,
    "RED_ONLY":   2,
...
def _mode_to_byte(mode_str: str) -> int:
    """Konversi mode navigasi string ke byte. Fallback ke 0xFF jika tidak dikenal."""
    return _MODE_MAP.get(str(mode_str).upper(), _MODE_DISCONNECT)


def _buoy_mode_to_detect_byte(buoy_mode: str) -> int:
    """
    Konversi buoy.mode string (dari vision_worker) ke detect byte enum.

    Returns
    -------
    0 — tidak ada buoy
    1 — kedua buoy (buoyred + buoygreen)
    2 — hanya buoyred
    3 — hanya buoygreen
    """
    return _DETECT_MAP.get(str(buoy_mode).upper(), 0)


def _build_packet(pwm: int, mode_byte: int, detect_byte: int) -> bytes:
    """
    Bangun paket binary 6 byte.

    Struktur:
      0xAA | PWM_LO | PWM_HI | MODE | DETECT | XOR(PWM_LO, PWM_HI, MODE, DETECT)

    Parameter
    ---------
    pwm         : nilai PWM servo dalam µs (1100–1900)
    mode_byte   : byte mode navigasi (0x00=MANUAL, 0x01=AUTO, 0x02=HOLD, 0xFF=DISCONNECT)
    detect_byte : enum status buoy — 0=tidak ada, 1=BOTH, 2=RED_ONLY, 3=GREEN_ONLY
    """
    lo   = pwm & 0xFF
    hi   = (pwm >> 8) & 0xFF
    csum = lo ^ hi ^ mode_byte ^ detect_byte
    return bytes([0xAA, lo, hi, mode_byte, detect_byte, csum])
...
class SerialWorker:
    ...
    def _run(self) -> None:
        ...
        last_mode_b = -1
        last_detect = -1      # -1 = belum pernah kirim status deteksi
        last_sent   = 0.0
        ...
        while not self._stop.is_set():
            ...
            if self._ser is None or not self._ser.is_open:
                try:
                    ...
                    # Reset agar kirim ulang seluruh nilai saat reconnect
                    last_pwm = last_mode_b = last_detect = -1
            ...
            # Konversi buoy.mode → detect byte enum (0–3)
            #   "NONE"       → 0 : tidak ada buoy
            #   "BOTH"       → 1 : buoyred + buoygreen terdeteksi
            #   "RED_ONLY"   → 2 : hanya buoyred
            #   "GREEN_ONLY" → 3 : hanya buoygreen
            buoy_mode   = data.get("buoy", {}).get("mode", "NONE")
            detect_byte = _buoy_mode_to_detect_byte(buoy_mode)

            now       = time.monotonic()
            changed   = (pwm != last_pwm) or (mode_byte != last_mode_b) or (detect_byte != last_detect)
            heartbeat = (now - last_sent) >= HEARTBEAT_INTERVAL

            # ── Kirim jika berubah ATAU heartbeat jatuh tempo ─────────────────
            if changed or heartbeat:
                try:
                    packet = _build_packet(pwm, mode_byte, detect_byte)
                    self._ser.write(packet)
                    last_pwm    = pwm
                    last_mode_b = mode_byte
                    last_detect = detect_byte
                    last_sent   = now
                    ...
                    self.store.update({
                        "serial": {
                            ...
                            "last_mode_b":  mode_byte,
                            "last_mode":    mode_str,
                            "last_detect":  detect_byte,   # 0=none 1=both 2=red 3=green
                        }
                    })
                except Exception as exc:
