...
func generate_path(
	...
	for target_wp_index: int in range(
		...
		# Gate awal tetap memakai dynamic cubic Bezier + collision checking.
		elif (
			target_wp_index >= 1
			and
			target_wp_index <= gate_count
		):

			var gate_index: int = target_wp_index - 1

			if (
				gate_index < red_balls.size()
				and
				gate_index < green_balls.size()
			):

				segment = create_dynamic_gate_curve(
					segment_start,
					previous_direction,
					red_balls[gate_index],
					green_balls[gate_index],
					SHIP_SPEED
				)

		# Waypoint non-gate fallback ke A*.
		else:
