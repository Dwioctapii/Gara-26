...
# Patch bug TensorRT & Numpy tanpa memicu FutureWarning.
...
def main():
    ...
    print(f"Membuka Kamera Atas (Source: {config.CAM_ATAS_SOURCE})...")
    ...
    # 2. Buka Kamera
    cap = cv2.VideoCapture(config.CAM_ATAS_SOURCE)
    ...
    while True:
        ...
        for box in result.boxes:
            ...
            if isinstance(names, dict):
                name = str(names.get(class_id, class_id)).upper()
            else:
                name = str(names[class_id]).upper()
