...
class Dashboard:
    ...
    def update(self, _):
        ...
        frame = self.state.latest_frame_rgb
        if self.state.frame_sequence != self._last_frame_sequence:
            self.cam_im.set_data(frame)
            self._last_frame_sequence = self.state.frame_sequence
        ...
        if self._pid_save_time and (time.time() - self._pid_save_time) > 3.0:
            ...
            # Figure-level text memiliki axes=None dan tidak aman untuk blit
            # pada backend GTK lama. Redraw hanya ketika status PID berubah.
            self.fig.canvas.draw_idle()

        now = time.monotonic()
        telemetry_interval = 1.0 / max(config.GUI_TELEMETRY_HZ, 0.1)
        if now - self._last_telemetry_update < telemetry_interval:
            return (
                self.robot,
                self.trace,
                self.arrow,
        ...
        if current_x is not None:
            self.robot.set_data([current_x], [current_y])
            self.trace.set_data(self.state.trace_x, self.state.trace_y)
            yaw = (
                -self.state.current_yaw_rad
                if self.arena.current == "B"
                else self.state.current_yaw_rad
            )
            color = "#00e5ff" if self.arena.current == "A" else "#ff007f"
            self.arrow.set_positions(
                (current_x, current_y),
                (
                    current_x + 1.2 * math.sin(yaw),
                    current_y + 1.2 * math.cos(yaw),
                ),
            )
        ...
        position = f"{current_x:.2f} m, {current_y:.2f} m" if current_x is not None else "0.00 m, 0.00 m"
        serial_snap = self.state.snapshot.get("serial", {})
        ...
        if values_text != self.values.get_text():
            self.values.set_text(values_text)
        self.values.set_color("#00e676" if self.state.is_armed else "#ff5252")
        return (
            self.robot,
            self.trace,
            self.arrow,
    ...
    def show(self):
        ...
        print(
            f"[GUI] Matplotlib backend={plt.get_backend()} "
            f"blit={'aktif' if config.GUI_BLIT else 'nonaktif'}"
        )
        self.animation = animation.FuncAnimation(
            self.fig,
            self.update,
            interval=max(1, int(1000.0 / max(config.GUI_FPS, 1.0))),
            blit=config.GUI_BLIT,
...
class GUIStateAdapter:
    def __init__(self, store, photo_dir):
        ...
        self.snapshot = store.gui_snapshot()
        self.lat0 = None
        self.lon0 = None
        ...
        self._raw_yaw_rad = 0.0
        self._current_yaw_rad = 0.0
        self._current_x = None
        self._current_y = None

        # Cache frame mencegah cvtColor dan set_data bila vision belum
        # menerbitkan frame baru.
        self._blank_frame = np.zeros(
            (config.CAMERA_HEIGHT, config.CAMERA_WIDTH, 3),
            dtype=np.uint8,
        )
        self._waiting_frame = self._camera_status_frame(
            "MENUNGGU KAMERA LIVE", (255, 210, 0)
        )
        self._stalled_frame = self._camera_status_frame(
            "FRAME KAMERA TERHENTI", (255, 80, 80)
        )
        self._frame_rgb = self._waiting_frame
        self._frame_sequence = ("waiting", 0)
        self._live_sequence = -1
        self._last_live_monotonic = None

    def _camera_status_frame(self, message, color):
        """Buat placeholder GUI; foto hasil misi tidak boleh jadi preview live."""
        ...
        (text_width, text_height), _ = cv2.getTextSize(
            message, font, scale, thickness
        )
        x = max(8, (config.CAMERA_WIDTH - text_width) // 2)
        y = max(text_height + 8, config.CAMERA_HEIGHT // 2)
        cv2.putText(frame, message, (x, y), font, scale, color, thickness,
                    cv2.LINE_AA)
        return frame

    def refresh(self):
        """Ambil tepat satu deep-copy state untuk satu tick GUI."""

        self.snapshot = self.store.gui_snapshot()
        self._raw_yaw_rad = self.snapshot.get("orientation", {}).get("z", 0.0)
        connected = bool(self.snapshot.get("connected"))
        if self.yaw_offset is None and connected:
            self.yaw_offset = self._raw_yaw_rad

        if self.yaw_offset is None:
            self._current_yaw_rad = 0.0
        else:
            relative = self._raw_yaw_rad - self.yaw_offset
            self._current_yaw_rad = math.atan2(
                math.sin(relative), math.cos(relative)
            )

        gps = self.snapshot.get("gps", {})
        lat = gps.get("lat") or 0.0
        lon = gps.get("lon") or 0.0
        self._current_x = self._current_y = None
        if lat != 0.0 and lon != 0.0 and self.arena_ref:
            heading_offset = self.yaw_offset if self.yaw_offset is not None else 0.0
            x, y = self.arena_ref.gps_to_xy(lat, lon, heading_offset, self)
            self._current_x, self._current_y = x, y
            if not self.trace_x or math.hypot(
                x - self.trace_x[-1], y - self.trace_y[-1]
            ) > 0.05:
                self.trace_x.append(x)
                self.trace_y.append(y)
                overflow = len(self.trace_x) - max(config.GUI_TRACE_MAX_POINTS, 1)
                if overflow > 0:
                    del self.trace_x[:overflow]
                    del self.trace_y[:overflow]
    ...
    def latest_frame_rgb(self):
        sequence, frame_bgr = self.store.latest_frame()
        if frame_bgr is not None:
            if sequence != self._live_sequence:
                self._frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
                self._live_sequence = sequence
                self._last_live_monotonic = time.monotonic()
                self._frame_sequence = ("live", sequence)

            stale_after = max(config.GUI_CAMERA_STALE_SECONDS, 0.1)
            if (
                self._last_live_monotonic is not None
                and time.monotonic() - self._last_live_monotonic > stale_after
            ):
                self._frame_sequence = ("stalled", sequence)
                return self._stalled_frame
            return self._frame_rgb

        # Jangan pernah memakai foto/atas.jpg sebagai preview: file tersebut
        # adalah hasil tangkapan misi dan bisa berumur berjam-jam.
        self._frame_sequence = ("waiting", 0)
    ...
    def raw_yaw_rad(self):
        return self._raw_yaw_rad

    @property
    def current_yaw_rad(self):
        return self._current_yaw_rad

    @property
    def current_lat(self):
        return self.snapshot.get("gps", {}).get("lat") or 0.0

    @property
    def current_lon(self):
        return self.snapshot.get("gps", {}).get("lon") or 0.0
    ...
    def gps_fix_type(self):
        return 3 if self.snapshot.get("gps", {}).get("fix") else 1

    @property
    def satellites_visible(self):
        return self.snapshot.get("gps", {}).get("satellites", 0)

    @property
    def current_sog(self):
        return self.snapshot.get("gps", {}).get("sog", 0.0) * 1.94384

    @property
    def current_cog(self):
        return self.snapshot.get("gps", {}).get("cog", 0.0)

    @property
    def ws_last_status(self):
        return "CONNECTED" if self.snapshot.get("connected") else "DISCONNECTED"

    @property
    def last_detected_class(self):
        return self.snapshot.get("detection", {}).get("label", "STANDBY")

    @property
    def current_mode(self):
        return self.snapshot.get("mode", "UNKNOWN")

    @property
    def is_armed(self):
        return self.snapshot.get("arm", "Disarmed") == "Armed"
